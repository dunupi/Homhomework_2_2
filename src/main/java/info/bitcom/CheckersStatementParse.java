...
public class  CheckersStatementParse {
    ...
    public List<Segment> parseSegments(String template){
        ...
        for (String s: strings){
            if (CheckersStatementAnalyzer.isVariable(s)){
    ...
    private int collectSegments(List<String> segs, String source) {
        Pattern pattern = Pattern.compile("\\$\\{[^}]*}");
        Matcher matcher = pattern.matcher(source);
        int index = 0;
        while (matcher.find()){
            addPrecedingPlainText(segs,source,matcher,index);
            addVariable(segs,source,matcher);
            index = matcher.end();
        }
        return index;
    }
